  ...
  for (const it of items) {
    ...
    if (Array.isArray(it.deal_burgers) && it.deal_burgers.length > 0) {
      for (const b of it.deal_burgers) {
        ...
        if (isSmashName(bn) || isVeganBurgerName(bn) || isCrispyChickenName(bn)) continue;
    ...
    if (isSmashName(name) || isVeganBurgerName(name) || isCrispyChickenName(name)) continue;
...
export function computeChefSummary(items: ReceiptOrderItem[]): ChefSummary {
  ...
  let smashPatties = 0;
  let smashDoubleCheesePatties = 0;
  let veganPatties = 0;
  ...
  for (const it of items) {
    ...
    if (it.deal_burgers && Array.isArray(it.deal_burgers)) {
      for (const b of it.deal_burgers) {
        ...
        else if (isSmashName(bn)) {
          smashPatties += qty;
          if (isSmashDoubleCheeseName(bn)) smashDoubleCheesePatties += qty;
        }
    ...
    else if (isSmashName(name)) {
      smashPatties += qty;
      if (isSmashDoubleCheeseName(name)) smashDoubleCheesePatties += qty;
    }
    ...
    // Smash extra patty topping ("+ קציצת סמאש" / "אקסטרה קציצת סמאש") →
...
// ---------- ACTIVE-ORDERS BON (per-order detail, oldest → newest) ----------
//
// Produces a single big bon listing every active order in the order they
...
const statusLabel = (s?: string | null): string => {
  ...
  if (s === "preparing") return "בהכנה";
  if (s === "ready") return "מוכנה";
  return "";
};
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .order-block:first-of-type { margin-top: 0; }
  .order-head {
    border-bottom: 1px dashed #000;
    padding-bottom: 1mm;
    margin-bottom: 1.5mm;
  ...
  .order-meta .cust { font-weight: 900; font-size: 12pt; }
  .order-meta .time { white-space: nowrap; }
  .order-meta .status {
    background: #000; color: #fff;
    padding: 0.3mm 1.5mm;
    border-radius: 2px;
...
export function printRoundSummary(orders: RoundOrder[]) {
...
// ---------- ROUND CHEF SUMMARY (aggregated only, no per-order detail) ----------
//
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  @media print { body { width: auto; padding: 1mm 2mm; } }
</style>
</head>
<body>
  <div class="head">סיכום סבב לטבח<small>${time}</small></div>
  <div class="meta">${orders.length} הזמנות פעילות · אגרגציה כוללת</div>
  ${bodyHtml}
  <div class="footer">סה״כ מנות להכנה בסבב</div>
</body>
</html>`;
}

export function printRoundChefSummary(orders: RoundOrder[]) {
  ...
  setTimeout(() => {
    try {
      ...
      console.warn("[round-chef-summary] print failed", e);
    }
  }, 250);
}
